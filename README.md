...
* **database/01_schema.sql**: Implementation of all database tables including Primary Keys, Foreign Keys, and constraints (CHECK or TRIGGER)[cite: 8, 9, 10, 11].
...
* The application **MUST** perform all data manipulations and retrievals by calling the Stored Procedures defined in Part 2[cite: 64, 66].

### 🔴 Error Handling
* Stored procedures must validate data and return **meaningful error messages** that clearly indicate the specific error (e.g., valid email formats or salary logic)[cite: 19, 20, 21].
* Generic error messages are not allowed[cite: 20].

---

## 📅 Deadlines & Deliverables
* **Session 5**: Preliminary Report submission[cite: 3].
* **Session 7**: Final Report submission and Presentation[cite: 4].
